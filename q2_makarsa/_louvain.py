...
def louvain_communities (
    ...
    def consensus_matrix(G, #Networkx graph object
        ...
        louvain_sum = louvain_sum.applymap(lambda x: x / num_partitions if x != 0 else 0)
        return louvain_sum
    ...
    def list_to_dataframe(
        ...
        for node in nodes_set:
            for comm in community_list:
                if node in comm:
                    l=[1 if x in comm else 0 for x in nodes_set]
                    df.loc[node]=l
                    break
        return df

    #Flag variable to change when the consenus is reached and counter to determine the number of iterations
    ...
    final_partition = pd.DataFrame({'OTUID': list(final_consensus.keys()), 'COMMUNITY': final_consensus.values()})
