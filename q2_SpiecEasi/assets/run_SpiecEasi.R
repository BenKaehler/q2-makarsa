...
option_list <- list(
  ...
  make_option("--pulsar.select",
    action = "store", default = TRUE, type = "character",
    help = "Perform model selection. Choices are TRUE/FALSE/'batch'  "
  ),
  make_option("--lambda.log",
    action = "store", default = TRUE, type = "character",
...
data <- t(data)
se.out <- spiec.easi(data,
  method = method, lambda.min.ratio = lambda.min.ratio,
