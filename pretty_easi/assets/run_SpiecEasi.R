...
if (method=='mb') {
    ...
    network <- adj2igraph(Matrix::drop0(getRefit(se.out)),
                  ...
                  vertex.attr = list(name=colnames(data)))
} else if (method=='glasso') {
    secor<- cov2cor(getOptCov(se.out))
    bm <- summary(triu(secor*getRefit(se.out), k=1)) 
    network <- adj2igraph(getRefit(se.out),edge.attr=list(weight=bm[,3]),vertex.attr = list(name=colnames(data)))
}else{
    print("Weighted graph only available for mb and glasso method")
    network <- adj2igraph(getRefit(se.out),vertex.attr = list(name=colnames(data)))
}
